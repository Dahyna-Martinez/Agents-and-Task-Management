...
def depth_limited_search(problem, limit=15):
    """[Figure 3.17]"""
...
def simulated_annealing(problem, schedule=exp_schedule()):
    ...
    for t in range(sys.maxsize):
        T = schedule(t)
        if T == 0:
            return current #Edited to return current instead of current.state
        neighbors = current.expand(problem)
        if not neighbors:
            return current #Edited to return current instead of current.state
...
def simulated_annealing_full(problem, schedule=exp_schedule()):
    ...
    for t in range(sys.maxsize):
        ...
        T = schedule(t)
        if T == 0:
            return states, len(states)  # Edited to return path and search cost
        neighbors = current.expand(problem)
        if not neighbors:
            return states, len(states)  # Edited to return path and search cost
        next_choice = random.choice(neighbors)
        delta_e = problem.h(next_choice) - problem.h(current) #problem.values() replaced with problem.h() and eliminated .state
        if delta_e > 0 or probability(np.exp(delta_e / T)):
            current = next_choice
